						...
						<div id='hero' className='grid '>
							...
							<div className='py-2 text-xs '>
								<div className='flex justify-between py-2'>
									<p>{num}</p>
									<Link href={`/category/${heroArticle.attributes.category.data.attributes.name}`}>
									<a>
									<p> {heroArticle.attributes.category.data.attributes.name}</p>
									</a>
									</Link>
									<p>
							...
							<div className='flex py-2 md:py-4 justify-between items-center'>
								...
								<Link href={`/author/${heroArticle.attributes.author.data.attributes.slug}`}>
											<a>
								<div className='flex flex-col items-end'>
									...
									<div className='flex items-center space-x-2 py-2'>
										<img
											...
											src={getStrapiMedia(
												heroArticle.attributes.author.data.attributes.picture
											  )}										/>
						...
						<div className='grid col-span-3 md:col-span-2 content-between'>
								...
								<div className='space-y-2 py-2'>
									...
									<div className='flex justify-between items-center'>
										...
										<Link href={`/author/${leftArticle.attributes.author.data.attributes.slug}`}>
											<a>
												<div className="flex items-center gap-2">
											<img
												className='object-cover w-6 h-6 md:w-8 md:h-8 rounded-full'
												src={getStrapiMedia(
													leftArticle.attributes.author.data.attributes.picture
												  )}											/>
											<p className='text-gray-700 dark:text-gray-200'>por</p>
											<p className='font-bold text-gray-600 dark:text-gray-300'>
											{leftArticle.attributes.author.data.attributes.name}
											</p>
											</div>
											</a>
											</Link>
											<p className='text-gray-400 dark:text-gray-500'>• em</p>
											<Link href={`/category/${leftArticle.attributes.category.data.attributes.name}`} >
											<a>
											<p className='text-gray-400 font-bold dark:text-gray-500'>
												{leftArticle.attributes.category.data.attributes.name}
											</p>
											</a>
											</Link>
										</div>
							...
							<div className='space-y-2'>
								...
								<div className='space-y-2 py-2'>
									...
									<div className='flex justify-between items-center'>
										<div className='flex items-center gap-2 text-[10px] md:text-xs'>
										<Link href={`/author/${rightArticle.attributes.author.data.attributes.slug}`}>
											<a>
												<div className="flex items-center gap-2">
											<img
												className='object-cover w-6 h-6 md:w-8 md:h-8 rounded-full'
												src={getStrapiMedia(
													rightArticle.attributes.author.data.attributes.picture
												  )}											/>
											<p className='text-gray-700 dark:text-gray-200'>por</p>
											<p className='font-bold text-gray-600 dark:text-gray-300'>
											{rightArticle.attributes.author.data.attributes.name}
											</p>
											</div>
											</a>
											</Link>
											<p className='text-gray-400 dark:text-gray-500'>• em</p>
											<Link href={`/category/${rightArticle.attributes.category.data.attributes.name}`}>
											<a>
											<p className='text-gray-400 font-bold dark:text-gray-500'>
												{rightArticle.attributes.category.data.attributes.name}
											</p>
											</a>
											</Link>
										</div>
